# train_roberta.py
"""Train standard RoBERTa ABSA. Run: python train_roberta.py"""
...
from src.data.roberta_dataset import RobertaABSADataset, load_csv
from src.data.roberta_dataset import ID2LABEL
...
EPOCHS      = 5
LR          = 2e-5
SAVE_PATH   = "checkpoints/roberta_absa_best.pt"
DEVICE      = "cuda" if torch.cuda.is_available() else "mps" if torch.backends.mps.is_available() else "cpu"

# ── Data ─────────────────────────────────────────────────────────────────────
train_ds = RobertaABSADataset(load_csv("data/final/train.csv"), max_len=MAX_LEN)
val_ds   = RobertaABSADataset(load_csv("data/final/val.csv"),   max_len=MAX_LEN)
